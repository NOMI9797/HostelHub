          ...
          <div>
            ...
            <div className="flex items-center space-x-4 text-gray-600">
              <div className="flex items-center space-x-1">
                ...
                <span>{hostel.city}, {hostel.area}</span>
              </div>
              <div className="flex items-center space-x-1">
                <Users className="w-4 h-4" />
                <span>{getGenderDisplay(hostel.genderSpecific)}</span>
              </div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold text-blue-600">{formatPrice(minPrice)}</div>
          ...
          <div className="space-y-4">
            {roomTypes.map((roomType, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-blue-600">{formatPrice(roomType.price)}</div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {facilities.map((facility) => (
              <div key={facility} className="flex items-center space-x-2 p-3 bg-gray-50 rounded-lg">
                ...
                <span className="text-gray-700 capitalize">{facility.replace(/([A-Z])/g, ' $1').trim()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-gray-700">{hostel.ownerName}</span>
          </div>
          <div className="flex items-center space-x-3">
            <Phone className="w-5 h-5 text-gray-500" />
            <span className="text-gray-700">{hostel.ownerPhone}</span>
          </div>
          <div className="flex items-center space-x-3">
            <Mail className="w-5 h-5 text-gray-500" />
            <span className="text-gray-700">{hostel.ownerEmail}</span>
          </div>
          {hostel.nearbyLandmark && (
            <div className="flex items-center space-x-3">
              <MapPin className="w-5 h-5 text-gray-500" />
              <span className="text-gray-700">Near {hostel.nearbyLandmark}</span>
            </div>
          )}
        </div>
      </div>
